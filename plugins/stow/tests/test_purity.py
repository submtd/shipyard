...
1. The pure rule-engine modules (stacks, blocks, config, detect, scaffold)
...
PURE_MODULES = ("stacks", "blocks", "config", "detect", "scaffold")
...
def test_pure_modules_list_covers_every_module_in_the_package():
    ...
    """
    on_disk = {p.stem for p in STOW_PKG.glob("*.py")} - {"__init__"}
