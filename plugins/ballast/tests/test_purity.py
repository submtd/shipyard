...
1. The pure rule-engine modules (stacks, config, detect, render, scaffold)
   must never import subprocess, os, or any networking module. All I/O
   lives outside these modules -- they take facts already gathered and
   decide; they must not be able to gather anything themselves.
...
BALLAST_PKG = PLUGIN_ROOT / "ballast"
...
PURE_MODULES = ("stacks", "config", "detect", "render", "scaffold")
...
def _all_source_files():
    files = list(BALLAST_PKG.glob("*.py"))
    files += list(HOOKS_DIR.glob("*.py"))  # empty if HOOKS_DIR doesn't exist yet
    return files
...
def test_pure_modules_list_covers_every_module_in_the_package():
    ...
    """
    on_disk = {p.stem for p in BALLAST_PKG.glob("*.py")} - {"__init__"}
